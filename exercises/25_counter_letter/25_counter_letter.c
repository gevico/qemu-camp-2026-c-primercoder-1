...
TreeNode* insert_or_update(TreeNode* root, char letter) {
    ...
    if(!root) return create_node(letter);
    if(letter==root->letter) root->count++;
    if(letter>root->letter) root->right = insert_or_update(root->right, letter);
    if(letter<root->letter) root->left = insert_or_update(root->left, letter);
    return root;
}
